...
public class MybatisLogUtil {
	private static final  Logger logger = LoggerFactory.getLogger(MybatisLogUtil.class);

	public String getQuery(SqlSession sqlSession, String queryXML, Object paramVO) {

		BoundSql boundSql = sqlSession.getConfiguration().getMappedStatement(queryXML).getSqlSource().getBoundSql(paramVO);

		// 쿼리문을 가져온다(이 상태에서의 쿼리는 값이 들어갈 부분에 ?가 있다)
		String sql = boundSql.getSql();

		// 쿼리실행시 맵핑되는 파라미터를 구한다
		Object param = paramVO;

		if (paramVO == null) { // 파라미터가 아무것도 없을 경우
			...
			if (param instanceof Integer || param instanceof Long || param instanceof Float
	        	 ...
	        	 */
	        	List<ParameterMapping> paramMapping = boundSql.getParameterMappings();
	        	Class<? extends Object> paramClass = param.getClass(); 		// logger.debug("paramClass.getName() : {}", paramClass.getName());
